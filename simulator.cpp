...
// to add persistence in the form of a virtual
// hard drive to a Harvard CPU archictecture. The harddrive is going to have 
// a DMA controller to facilitate moving data
// between main memory and the drive itself
//-----------------------------------------
#include <stdio.h>
#include <fstream>
#include <string>
...
// filename of the binary file we will use for the harddrive
...
// main memory locations mapped to DMA registers
#define HD_STATUS        0
#define HD_ADDRESS       1
#define HD_MEM_ADDRESS      2
...
struct STATE
{
  ...
  // DMA registers needed for DMA transfers
  unsigned short DMA_status;
  unsigned short DMA_hd_address;
  unsigned short DMA_memory_address;
};
...
int removeLRU()
{
  ...
  // reset the valid bit and ref_count to 0 to indicate that the cache block is now empty so data can be loaded into it
  // no need for dirty bits since we now use write-through
...
// this function invalidates a cache block if a DMA transfer has caused that a word in that cache block to be updated in main memory
void cache_snoop()
{
  unsigned long tag = addr2tag(state.DMA_memory_address); // extract the tag
  int block_id;
  
  // checks the cache for the tag
  // if the tag is in the cache, set the valid bit to 0, that is invalidate that cache block
...
Phase cache_write()
{
  ...
  if ( state.MAR < (DATA_SIZE*BLOCK_SIZE*2) )
  {
    ...
    // we are now using write through
    // therefore, when we write word already contained in the cache to the cache, we write word to main memory simultaneously
    // this updates the contents of main memory to ensure main memory does not contain old data
  ...
  switch( opcode() )
  {
    ...
    case MOVE_OPCODE:
      ...
      // otherwise, fetch from memory
        ...
        // Intercept DMA register access
        // Assign appropriate DMA register to the MDR, so we can copy it to the destination register in the write_back phase
        if ( state.MAR <= HD_MEM_ADDRESS )
        {
          switch ( state.MAR )
          {
            case HD_ADDRESS:
              state.MDR = state.DMA_hd_address;
              break;
              
            case HD_MEM_ADDRESS:
              state.MDR = state.DMA_memory_address;
              break;
              
            case HD_STATUS:
              state.MDR = state.DMA_status;
              break;
          }
        }
        else{
  ...
  switch( opcode() )
  {
    ...
    case MOVE_OPCODE:
      ...
      if ( mode() & 0x04 ) {
        ...
        // Literal or Register to memory
        // Intercepts DMA register access when the memory address to be written to is either 0, 1, 2
        // Store the value contained in the MDR to the appropriate DMA/HD register
        // This sets one of the DMA registers in preparation for the interrupt phase
        // The Status register must be set with words to transfer before the DMA controller can actually do any transfer in the interrupt phase
        if ( state.MAR <= HD_MEM_ADDRESS )
        {
          switch ( state.MAR )
          {
            case HD_ADDRESS:
              state.DMA_hd_address = state.MDR;
              break;
              
            case HD_MEM_ADDRESS:
              state.DMA_memory_address = state.MDR;
              break;
              
            case HD_STATUS:
              state.DMA_status = state.MDR;
              break;
          }
        }
        else {
...
// this is where we perform DMA transfers if we need to
// what is done in this function is essentially the job of a DMA controller
// we implement the DMA transfers here as 'burst' transfers 
// each time this Phase is reached, if there are words to transfer, up to but not more than 4 words
// will be transferred between main memory and the hard drive depending on the operation(read or write)
Phase interrupts() {
  Phase rc = FETCH_INSTR;

  int word_count = 4;     // maximum number of words that can be transferred with each DMA
  unsigned short transfer_count = state.DMA_status >> 1;  // number of words left to transfer in the status register
  bool write = false; // boolean variable for determing whether we have a read or write operation
  unsigned long tag; // the address in main memory to be written to or read from
  unsigned short offset;  // the block in a specific address in main memory to be written to or read from
  
  // if the Least Signficant Bit of the status register is set, we have a write operation
  // if not, we have a read operation
  if (state.DMA_status & 0x0001) {
    write = true;
  }

  // check to see if we have words to transfer first!
  if( transfer_count > 0  )
  {
    // transfer words based on the operation we have(read or write)
    // implemented as burst transfers, transferring a maximum of 4 words
    while (word_count > 0 && (transfer_count > 0))
    { 
      tag = addr2tag(state.DMA_memory_address);   // get the appropriate memory address since our data memory array is a 3D array of blocks
      offset = addr2offset(state.DMA_memory_address);   // get the appropriate block in a given address in main memory 

      // if we have a write operation, write to the hard drive from the appropriate memory location
      if (write) {
        hd[state.DMA_hd_address][0] = data[tag][offset][0];
        hd[state.DMA_hd_address][1] = data[tag][offset][1];
      
      // read from the hard drive into the appropriate main memory location
      } else {
        data[tag][offset][0] = hd[state.DMA_hd_address][0];
        data[tag][offset][1] = hd[state.DMA_hd_address][1];
        
        // use write-invalidate to check cache to invalidate cache block containing data that
        // has been updated in main memory
        cache_snoop();
      }
      // Increment the DMA hard drive address register and DMA Memory address register to transfer the next word
      state.DMA_hd_address++;
      state.DMA_memory_address++;

      // Decrement transfer_count and word_count since we can only transfer up to 4 words at a time
      transfer_count--;
      word_count--;
    }
  }

  // set the DMA register to contain the number of words left to transfer and leave the read or write bit to whatever it was before
  // it started transferring
  state.DMA_status = (transfer_count << 1) | write;
...
// this function writes the contents of the hard drive array into the hard drive binary file
void write_hd()
{
...
void initialize_system()
{
  ...
  // initialize DMA/hard drive registers
  state.DMA_status = 0;
  state.DMA_hd_address = 0;
  state.DMA_memory_address = 0;
  ...
  // fill our hard drive with invalid data
  for ( i=0 ; i<HD_SIZE ; i++ )
  {
    hd[i][0] = MEM_FILLER;
    hd[i][1] = MEM_FILLER;
...
bool load_files( const char *code_filename, const char *data_filename )
{
  ...
  // load data from the hard_drive if the hard drive exists
  if( hd_file )
  {
...
int main (int argc, const char * argv[])
{
  ...
  if ( load_files( argv[1], argv[2] ) )
  {
    ...
    print_memory();
    // write the hard drive array to the hard drive file
    write_hd();
  }
}
